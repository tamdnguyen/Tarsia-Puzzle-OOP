...
//
//
//  /** Returns the [[TriGridPos]] that contains coordinate (x,y). */
//  // TODO: investigate bug when remove edgeLength
//  def pickTri(x: Double, y: Double): TriGridPos =
//    TriGridPos(ceil((1 * x - this.coeff / 3 * y) / this.edgeLength).toInt,
//      floor((this.coeff * 2 / 3 * y) / this.edgeLength + 1).toInt,
//      ceil((-1 * x - this.coeff / 3 * y) / this.edgeLength).toInt)
//
//
//  /** Returns the array of this t */
//  def tri_neighbours: List[(Double, Double, Double)] = {
