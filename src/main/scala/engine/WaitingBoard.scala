...
/** An instance of the class `RobotWorld` represents a mutable, two-dimensional world
  ...
  * @see [[Triangle]] */
class WaitingBoard(floorWidth: Int, floorHeight: Int) extends TriGrid[Triangle](floorWidth+2, floorHeight+2):
  ...
  def addRobot(initialLocation: GridPos, initialFacing: CompassDir) =
    val newBot = RobotBody(this, initialLocation, initialFacing) // create new robot
    this.robots = this.robots :+ newBot // add robot to the list
    this.elementAt(initialLocation).addRobot(newBot) // adding robot to the square
    newBot
  ...
  def nextRobot: Option[RobotBody] =
    ...
    if this.robotList.nonEmpty then
      Option(this.robots(this.turn))
    else
      None
  ...
  def advanceTurn() =
   // TODO: replace with a proper implementation
    ...
    if this.robotList.nonEmpty then
      this.turn = (this.turn + 1) % this.robotList.size
  ...
  def advanceFullRound() =
    // TODO: replace with a proper implementation
    1 to this.robotList.size foreach( _ => this.advanceTurn() )
